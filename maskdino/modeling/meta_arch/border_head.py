...
class BorderSemanticHead(nn.Module):
    """
    Lightweight dense head for manga border prediction.

    It fuses three sources:
    - pixel-decoder mask features for global structure
    - low-level backbone features for edges
    - the RGB image tensor for local line detail
    """
    ...
    def __init__(
        ...
        hidden_dim: int = 128,
        image_hidden_dim: int = 32,
        full_res_hidden_dim: int = 32,
    ):
        super().__init__()
        mid_hidden_dim = max(hidden_dim // 2, 64)
        ...
        self.image_s1_stem = nn.Sequential(
            ...
            _make_group_norm(full_res_hidden_dim, max_groups=8),
            nn.ReLU(inplace=True),
        )
        ...
        self.predictor = nn.Conv2d(full_res_hidden_dim, 1, kernel_size=1)
